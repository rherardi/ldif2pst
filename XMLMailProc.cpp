...
CMailbox* CXMLMailProc::getMailboxInfo()
{
	...
	if (theFoldersNode != NULL)
	{
		...
		XalanNode* formsNode = attrs->getNamedItem(XalanDOMString(XMLATTRIBUTE_MESSAGEFORMS));
		if (formsNode != NULL)
			info->SetMessageForms(formsNode->getNodeValue().c_str());
		XalanNode* skipFormsNode = attrs->getNamedItem(XalanDOMString(XMLATTRIBUTE_SKIPMESSAGEFORMS));
		if (skipFormsNode != NULL)
			info->SetSkipMessageForms(skipFormsNode->getNodeValue().c_str());
