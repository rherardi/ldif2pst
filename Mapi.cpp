...
#if defined(UNICODE)
	if (szDataDir != NULL && _tcslen(szDataDir) > 0)
		wsprintf(pszDataFile, _T("%s%s%s"), szDataDir, szDataFile, pstDefaultSuffix);
	else
		wsprintf(pszDataFile, _T("%s%s"), szDataFile, pstDefaultSuffix);
//	wsprintfW(pszDataFile, L"%S", "d:/temp/nmail2pst/Chaliapinas_Andrejus_hqwww01_Infosana_20070601.pst");
#else
	if (szDataDir != NULL && _tcslen(szDataDir) > 0)
		wsprintfW(pszDataFile, L"%S%S%S", szDataDir, szDataFile, pstDefaultSuffix);
	else
		wsprintfW(pszDataFile, L"%S%S", szDataFile, pstDefaultSuffix);
#endif
    rgval[0].Value.lpszW = pszDataFile;
	INI_LogWriteWithResourcePrefix(IDS_LOG_MAPI_PROFILECR, pszDataFile);
//	_tprintf(_T("-->pszDataFile %s\n"), pszDataFile);

    rgval[1].ulPropTag = PR_DISPLAY_NAME_W; // check also PR_PROVIDER_DISPLAY_W
#if defined(UNICODE)
	wsprintf(pszFolderName, _T("%s"), szDataFile);
//	wsprintfW(pszFolderName, L"%S", "Chaliapinas_Andrejus_hqwww01_Infosana_20070601");
#else
	wsprintfW(pszFolderName, L"%S", szDataFile);
#endif
	// Eliminate any slash/back slash we may have in case of already presented PST file
	lastSlash = _tcsrchr(pszFolderName, '\\');
	lastBackSlash = _tcsrchr(pszFolderName, '/');
	lastDot = _tcsrchr(pszFolderName, '.');
	if (lastDot != NULL)
		*lastDot = _T('\0');
	if (lastBackSlash != NULL && ((lastSlash != NULL && lastBackSlash > lastSlash) || (lastSlash == NULL)))
		lastSlash = lastBackSlash;
	if (lastSlash == NULL) {
	    rgval[1].Value.lpszW = pszFolderName;
	}
	else {
	    rgval[1].Value.lpszW = lastSlash + 1;
	}

//    rgval[1].Value.lpszW = pszFolderName;
	INI_LogWriteWithResourcePrefix(IDS_LOG_MAPI_PROFILECR, rgval[1].Value.lpszW);
//	_tprintf(_T("-->pszFolderName %s\n"), pszFolderName);
...
//    cout<<" hRes = 0x"<<hex<<hRes<<dec<<endl;
	INI_LogWriteWithResourcePrefixNum(IDS_LOG_MAPI_PROFILECREATEERR, GetScode(hRes));
	result = FALSE;
...
error:
//    cout<<" hRes = 0x"<<hex<<hRes<<dec<<endl;
	INI_LogWriteWithResourcePrefixNum(IDS_LOG_MAPI_PROFILEDELETEERR, GetScode(hRes));
    result = FALSE;
